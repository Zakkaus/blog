...
description: "2025 年最新 Apple Silicon Mac (M1/M2) Gentoo Linux 安裝指南，基於 Asahi Linux 專案，包含 Live USB 製作、分割槽、核心安裝及桌面環境配置。M3/M4/M5 晶片暫不支援。"
aliases:
  - /posts/gentoo-m-series-mac-arm64/
---
...
- **官方版本**：
  ...
  - **推薦使用**：`install-arm64-asahi-20250603.iso`（2025 年 6 月版本，已測試穩定）

  - **可能無法啟動**：`install-arm64-asahi-20251022.iso`（2025 年 10 月版本）在某些裝置（如 M2 MacBook）上可能無法正常啟動
...
```
> **提示**：Apple Silicon 的 Wi-Fi 已包含在核心中，應可正常運作。若不穩定，嘗試連線 2.4 GHz 網路。
**（可選）SSH 遠端操作**：
```bash
passwd                     
# 設定 root 密碼
...
**從這裡開始遵循 [ARM64 Handbook](https://wiki.gentoo.org/wiki/Handbook:ARM64)** 直到核心安裝章節。

## 4.1 下載並展開 Stage3
```bash
cd /mnt/gentoo
# 透過官方 latest-stage3 txt 檔案自動取得當前路徑（wget 不支援遠端萬用字元，必須用此方式）
...
更多映像源選項參考：[映像列表](https://www.gentoo.org/downloads/mirrors/)
...
## 8.3 更新韌體
macOS 系統更新時會包含韌體更新。**建議保留 macOS 分割槽**以便獲取最新韌體。
---
...
2. 確認使用正確的 LUKS UUID：`blkid /dev/nvme0n1p6`
