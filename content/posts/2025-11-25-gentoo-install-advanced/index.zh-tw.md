...
- `candy`：美化輸出
- `ccache`：編譯快取（需安裝 `dev-util/ccache`）
...
```bash
emerge --ask llvm-runtimes/clang-runtime
...
對於手動編譯，建議將**關鍵驅動**（如檔案系統、磁碟控制器、網路卡）直接編譯進核心（選擇 `[*]` 或 `<*>` 即 `=y`），而不是作為模組（`<M>` 即 `=m`）。這樣可以避免 initramfs 缺失模組導致無法啟動的問題。
...
3. **裝置驅動**：
   - `Device Drivers → Network device support` (網路卡驅動)
