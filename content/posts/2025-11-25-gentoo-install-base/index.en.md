...
<p>Last verified: 25 November 2025</p>
...
<details>
<summary><b>genfstab flags</b></summary>

<div>

| Flag | Identifier | Notes |
|------|------------|-------|
| `-U` | filesystem **UUID** | Recommended |
| `-L` | filesystem **LABEL** | Requires preset labels |
| `-t PARTUUID` | GPT **PARTUUID** | GPT only |
...
[`genfstab`](https://wiki.archlinux.org/title/Genfstab) detects every filesystem under the target mount point. According to its [source](https://github.com/glacion/genfstab/blob/master/genfstab) it explicitly supports:
...
<details>
<summary><b>LUKS encrypted partition configuration</b></summary>
...
lsblk
# You should see something like: nvme0n1p3 → cryptroot → mount point
...
**Q: Why can't I use the LUKS container UUID?**
A: A LUKS container holds the encrypted raw bytes — the OS can't read a filesystem out of it. After unlock, the `/dev/mapper/xxx` device exposes a readable filesystem with its own UUID.

**Q: Is `discard=async` safe on top of LUKS?**
A: LUKS2 + `discard` is generally safe. If you're particularly security-conscious, drop the option (at the cost of some SSD performance).
