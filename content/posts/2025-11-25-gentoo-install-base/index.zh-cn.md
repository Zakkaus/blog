...
根据[**下载页面**](https://www.gentoo.org/downloads/) 提供的方式获取下载链接
...
```bash
# 导入 Gentoo 官方发布签名公钥。
# 推荐方式：使用 Live media 或 sys-apps/openpgp-keys-gentoo-release 提供的本地副本
gpg --import /usr/share/openpgp-keys/gentoo-release.asc
# 备用方式（没有本地副本时，从 keyserver 取）：
...
当前 Gentoo Live media 默认启用 NetworkManager，有线一般自动 DHCP，无线可用一个 TUI 配好。

**最简单的方式 — NetworkManager（Live media 默认）：**
...
如需手动配置（或使用旧版没有 NetworkManager 的 Minimal ISO），参考下面两节。
...
**wpa_supplicant**（替换为你的接口、SSID、密码）：
...
映射名 `cryptroot` 后续 dracut/fstab/引导器章节会重复使用，若你改名请保持一致。
...
# installkernel USE flag 要与你用的引导器对应，这样 dist-kernel 升级时会自动重生引导器配置。择一：
...
通过编辑 sudoers 文件，允许 `wheel` 组的用户以 root 身份执行命令：
