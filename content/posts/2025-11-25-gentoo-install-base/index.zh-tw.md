...
一個最小化的 Gentoo 基礎系統壓縮套件。它包含了構建完整系統的基礎工具鏈（編譯器、庫等）。你將解壓它到硬碟上，作為新系統的"地基"。
...
```bash
# 匯入 Gentoo 官方發布簽名公鑰。
# 推薦方式：使用 Live media 或 sys-apps/openpgp-keys-gentoo-release 提供的本地副本
gpg --import /usr/share/openpgp-keys/gentoo-release.asc
# 備用方式（沒有本地副本時，從 keyserver 取）：
...
當前 Gentoo Live media 預設啟用 NetworkManager，有線一般自動 DHCP，無線可用一個 TUI 配好。

**最簡單的方式 — NetworkManager（Live media 預設）：**

```bash
nmtui   # 互動式：選擇「Activate a connection」或「Edit a connection」
```
...
若需要手動配置（或使用舊版沒有 NetworkManager 的 Minimal ISO），請參考下面兩節。
...
**wpa_supplicant**（替換為你的介面、SSID、密碼）：
...
對映名 `cryptroot` 後續 dracut/fstab/載入器章節會重複使用，若你改名請保持一致。
...
└─nvme0n1p3      259:9    0 926.5G  0 part
  └─cryptroot    253:0    0 926.5G  0 crypt /mnt/gentoo
```
...
**前提條件**：在執行 `genfstab` 之前，必須確保所有分割槽已正確掛載（包括 Btrfs 子卷和已解密的 LUKS 分割槽）。
...
只要 Btrfs 子卷已正確掛載，`genfstab -U` 會自動識別並產生包含 `subvol=` 的配置。
...
# installkernel USE flag 要與你用的載入器對應，這樣 dist-kernel 升級時會自動重生載入器配置。擇一：
#   GRUB           → 'sys-kernel/installkernel dracut grub'
#   systemd-boot   → 'sys-kernel/installkernel dracut systemd systemd-boot'
#   Limine / 其他  → 'sys-kernel/installkernel dracut'（載入器手動維護）
...
透過編輯 sudoers 檔案，允許 `wheel` 群組的使用者以 root 身分執行命令：
...
```
*（僅限多系統使用者）務必檢查輸出中是否包含 "Found Windows Boot Manager..." 或其他系統的引導項*
