...
Hyprland lives in the `hyproverlay` overlay (not the main Portage tree). Enable the overlay, accept its keywords, then install:

```bash
emerge --ask app-eselect/eselect-repository
eselect repository enable hyproverlay
emaint sync -r hyproverlay
# Hyprland and its deps are ~amd64; accept the overlay's keywords:
...
Hyprland tracks recent Wayland and graphics driver versions. Read the [Gentoo Wiki](https://wiki.gentoo.org/wiki/Hyprland) for detailed configuration.
...
Rime is a universal input engine that supports Chinese, Japanese, Korean, Vietnamese, and other languages through installable schemas. For Wayland, configure the environment variables below.
...
3. **Confirm Setup Mode is on**:
    ```bash
    sbctl status
    ```
    Look for `Setup Mode: Enabled`. If `Disabled`, go back to step 2.
...
5. **Sign kernel and bootloader**:
    ...
    ```
6. **Verify everything is signed**:
    ```bash
    sbctl verify
    ```
7. **Reboot, then confirm Secure Boot is active**:
    ```bash
    sbctl status
    ```
    Expect `Secure Boot: Enabled` and `Setup Mode: Disabled`.
