...
Hyprland 位于 `hyproverlay` overlay（不在主 Portage 树中）。启用 overlay、接受 keyword、再安装：
...
Hyprland 跟进较新的 Wayland 与显卡驱动版本，详细配置请阅读 [Gentoo Wiki](https://wiki.gentoo.org/wiki/Hyprland)。
...
3. **确认 Setup Mode 已开启**：
    ...
    sbctl status
    ```
    应看到 `Setup Mode: Enabled`，若是 `Disabled` 请回到第 2 步。
4. **创建并注册密钥**：
    ```bash
    sbctl create-keys
...
6. **验证全部已签名**：
    ...
    sbctl verify
    ```
7. **重启后确认 Secure Boot 已启用**：
    ```bash
    sbctl status
    ```
    应看到 `Secure Boot: Enabled` 与 `Setup Mode: Disabled`。
