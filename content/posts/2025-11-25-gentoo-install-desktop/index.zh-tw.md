...
Hyprland 位於 `hyproverlay` overlay（不在主 Portage 樹中）。啟用 overlay、接受 keyword、再安裝：
...
Hyprland 跟進較新的 Wayland 與顯示卡驅動版本，詳細配置請閱讀 [Gentoo Wiki](https://wiki.gentoo.org/wiki/Hyprland)。
...
3. **確認 Setup Mode 已開啟**：
    ...
    sbctl status
    ```
    應看到 `Setup Mode: Enabled`，若是 `Disabled` 請回到第 2 步。
4. **建立並註冊金鑰**：
    ```bash
    sbctl create-keys
...
5. **簽名核心與載入程式**：
    ```bash
    # 自動查詢並簽名所有已知檔案 (包括核心、systemd-boot 等)
    sbctl sign-all
    # 或者手動簽名 (例如 GRUB)
...
6. **驗證全部已簽名**：
    ...
    sbctl verify
    ```
7. **重啟後確認 Secure Boot 已啟用**：
    ```bash
    sbctl status
    ```
    應看到 `Secure Boot: Enabled` 與 `Setup Mode: Disabled`。
...
> 根據 Gentoo 官方公告，Gentoo 已停止為第三方倉庫提供快取映像支援。從 2025-10-30 起，所有第三方倉庫（包括 gentoo-zh）的映像配置將從官方倉庫列表中移除。
